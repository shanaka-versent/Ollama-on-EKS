...
- **Region-level snapshot** — volume is created in whichever AZ Karpenter places the GPU node (enables multi-AZ spot)
- **Lazy-loading** — EBS snapshots use S3-backed lazy loading. The high-throughput gp3 config (400 MB/s, 6000 IOPS) compensates for first-read latency
- **Cold start ~3 min** (snapshot restore + model load to VRAM) vs 15-25 min if downloading from internet
