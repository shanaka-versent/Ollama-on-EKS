...
```
.
├── deploy.sh                     # Entry point — terraform apply + post-setup
├── destroy.sh                    # Entry point — full teardown
├── claude-switch.sh              # Switch Claude Code between remote / local / Kong
...
│   ├── outputs.tf
│   ├── providers.tf
│   ├── README.md                 # Terraform variable reference
│   └── modules/
│       ├── vpc/                  # VPC, subnets, NAT
│       ├── iam/                  # IAM roles
│       ├── eks/                  # EKS cluster + node groups
│       ├── argocd/               # ArgoCD GitOps bootstrap
│       └── lb-controller/        # AWS LB Controller
├── argocd/
...
│       └── 11-httproutes.yaml
├── k8s/
│   ├── namespaces.yaml           # ollama + istio-ingress namespaces
│   ├── gateway.yaml              # Istio Gateway → internal NLB
│   ├── httproutes.yaml           # HTTPRoutes → Ollama :11434
│   ├── ollama/                   # Deployment, Service, NetworkPolicy, StorageClass, PVC
│   └── model-loader/             # Job: pulls qwen3-coder:30b to EBS
├── scripts/
│   ├── 01-setup.sh               # Post-terraform orchestrator
│   ├── 02-generate-certs.sh      # TLS secret for Istio Gateway
│   ├── 03-setup-cloud-gateway.sh # Kong Konnect control plane + TGW setup
│   └── 04-post-setup.sh          # NLB discovery + Kong config sync
├── deck/
│   └── kong.yaml                 # Kong gateway declarative config
└── docs/
    └── architecture.png
```
