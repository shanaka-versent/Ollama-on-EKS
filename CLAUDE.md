...
Layer 2 — EKS Control Plane with Auto Mode enabled. Built-in EKS Auto Mode `system` pool (c6g.large Graviton on-demand) handles infra (CoreDNS, Istio, ArgoCD, Prometheus). Two custom Karpenter NodePools: app pool (c6g.xlarge Graviton on-demand, ARM64, tainted `workload-type=app:NoSchedule` — Open WebUI) + GPU pool (g5 spot, on-demand fallback, flex ceiling allows g5.2xlarge and g5.12xlarge, multi-AZ). EBS CSI Driver + AWS LB Controller managed by Auto Mode. App nodes in single AZ (`ap-southeast-2a`); GPU nodes multi-AZ for spot availability.
...
- CloudFront Function (`auth_redirect`) runs on `viewer-request` event — checks for `token` cookie, redirects unauthenticated users to `/auth/login.html` (custom login page). Excludes: `/oauth/*`, `/_app/*`, `/static/*`, `/api/*`, `/v1/*`, `/ws/*`, `/portal/*`, `/auth/*`, favicons
- **WebSocket disabled (`ENABLE_WEBSOCKET_SUPPORT=false`)** — CloudFront VPC Origins don't support WebSocket protocol upgrade. Socket.io uses HTTP long-polling instead. Without this, socket.io silently fails and chat falls back to SSE streaming which triggers a known frontend JSON parse bug (open-webui#19037). The `/ws/*` CloudFront Function exclusion lets socket.io polling bypass cookie auth (socket.io has its own token-based auth in the handshake)
...
- EKS Auto Mode with built-in + custom NodePools — Auto Mode manages Karpenter, NVIDIA plugin, EBS CSI, LB controller. Built-in `system` pool (c6g.large Graviton) handles all infra workloads (CoreDNS, Istio, ArgoCD, Prometheus). Two custom pools: app (c6g.xlarge Graviton on-demand, tainted for Open WebUI) and GPU (g5 spot with on-demand fallback, flex ceiling allows g5.2xlarge + g5.12xlarge, multi-AZ). App nodes in single AZ (`ap-southeast-2a`); GPU multi-AZ for spot availability
- System and app nodes on-demand only — CoreDNS, Istio, Prometheus, ArgoCD, Open WebUI cannot tolerate spot interruptions. GPU nodes use spot (inference tolerates 2-3 min interruptions). `GPUOnDemandFallback` alert fires if GPU falls back to on-demand
- Separate app NodePool for Open WebUI (share_nodepool=no pattern) — Open WebUI runs on its own Graviton ARM64 nodes (c6g.xlarge), isolated from infrastructure via `workload-type=app:NoSchedule` taint. Keeps infra pods on built-in system nodes and user-facing apps on dedicated app nodes. Open WebUI multi-arch Docker image supports ARM64/Graviton
